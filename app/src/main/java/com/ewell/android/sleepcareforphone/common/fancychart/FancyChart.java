...
public class FancyChart extends View {
	...
	private static int HORIZONTAL_LEGEND_MARGIN_TOP = 5;
	...
	protected void onDraw(Canvas canvas) {
		...
		for(ChartData data : chartData) {
			...
			for(Point point : points) {
				if(points.size()==1){
					point.canvasX = VERTICAL_LEGEND_WIDTH + VERTICAL_LEGEND_MARGIN_RIGHT + chartStyle.getChartPaddingLeft();
				}
				if(point.isSelected) {
					// Draw selected point
	...
	private void drawHorizontalLinesAndLegend(Canvas canvas) {
		...
		for(AxisValue value : reduced) {
			...
			canvas.drawLine(VERTICAL_LEGEND_WIDTH+VERTICAL_LEGEND_MARGIN_RIGHT , y, getWidth()-30, y, paint);
